<x-app-layout>
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="post" action="{{ route('owner.products.store')}}" >
                        ...
                        <div class="-m-2">
                            ...
                            <x-select-image :images="$images" name="image1" />{{--imagesはログインしているオーナーの画像一覧(imagesテーブル)--}}
    ...
    <script>
        'use strict'
        const images = document.querySelectorAll('.image')//class="image"のタグの情報をまとめてimagesという変数に格納する(resources\views\components\select-image.blade.php)
        
        images.forEach( image =>  {//x-select-imageのimg class="image"タグの中に、data-○○という属性をそれぞれ付けることで、phpからJSに情報を渡す
            image.addEventListener('click', function(e){//引数eでタグ内の情報を取得(e.target.dataset.○○で情報を受け取る)
                const imageName = e.target.dataset.id.substr(0, 6)
                const imageId = e.target.dataset.id.replace(imageName + '_', '')
                const imageFile = e.target.dataset.file
                const imagePath = e.target.dataset.path
                const modal = e.target.dataset.modal
                document.getElementById(imageName + '_thumbnail').src = imagePath + '/' + imageFile//document.getElementById() でタグのidを取得し、idがついてるタグのsrc属性に 画像のパスを格納する
                document.getElementById(imageName + '_hidden').value = imageId
            }, )//imageタグそれぞれに、クリックした時の処理を追加
        })  
    </script>
